...
class AndSpecification<T, SpecA: Specification, SpecB: Specification> : Specification where T == SpecA.T, T == SpecB.T{
    let first: SpecA
    let second: SpecB
    init(_ first: SpecA, second: SpecB) {
        self.first = first
        self.second = second
    }
    func isSatisfied(_ item: T) -> Bool {
...
func main(){
    ...
    print("Large blue item using new better filter and AndSpecification")

    for p in bf.filter(products, AndSpecification(ColorSpecification(.blue), second: SizeSpecification(.large))){
